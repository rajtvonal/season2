const rulesPath = window.RAJTVONAL_RULES_DATABASE_PATH || "../../database/F1";
...
function isLevel1(line) { return /^\d+\.\s/.test(line); }
function isLevel2(line) { return /^\d+\.\d+\s/.test(line); }
function isLevel3(line) { return /^\d+\.\d+\.\d+$/.test(line); }

async function initRulesPage() {
  const container = document.querySelector(".container.rule");
  if (!container) return;
  try {
    const response = await fetch(`${rulesPath}/rules.txt`);
    const text = await response.text();

    generateRules(text);
  } catch (error) {
    console.error("Rules file loading failed:", error);
  }
}
