...
(() => {
    ...
    for(let i = 0; i < COLUMN_COUNT; i++) {
        ...
        columns.push({
            index: i,
            size: 48 - (t * 38),        // méretcsökkenés
            spacingX: 55 + (t * 20),    // oszlopköz
            spacingY: 52 - (t * 12),    // sorköz
            missingChance: 0.05 + Math.pow(t, 1.15) * 0.45  // szétesés
        });
    }
    ...
    function drawSide(isLeft, time) {
        ...
        columns.forEach(col => {
            ...
            if(col.index > 1) {
                move = Math.sin(time * 0.00012) * Math.pow(col.index - 1, 1.3) * 2.2;
            }

            const x = isLeft ? currentOffset - move : triangleCanvas.width - currentOffset - col.size + move;
            const rows = Math.ceil(triangleCanvas.height / col.spacingY) + 2;
            let consecutiveMissing = 0;

            for(let row = 0; row < rows; row++) {
                const seed = (row * 92821) ^ (col.index * 68917);
                ...
                } else { consecutiveMissing = 0; }
                if(skip) { continue; }

                const y = row * col.spacingY;

                let rightDirection;
                let leftDirection;
                if(col.index % 2 === 0) {
                    rightDirection = row % 2 === 0 ? "right" : "left";
                    leftDirection = row % 2 === 0 ? "left" : "right";
                } else {
                    rightDirection = row % 2 === 0 ? "left" : "right";
                    leftDirection = row % 2 === 0 ? "right" : "left";
                }

                drawTriangle(x, y, col.size, isLeft ? rightDirection : leftDirection);
            }
    ...
    function animate(time) {
        ...
        triangleCtx.fillStyle = "rgba(219,10,64,0.3)";
