...
def load_drivers( file_path = 'C:\\Users\\Dani\\Desktop\\RAJTVONAL\\season2\\database\\SM\\BOP_input.csv' ):
    drivers = []
    rows_done = 0
...
def save_bops(results, file_path = 'C:\\Users\\Dani\\Desktop\\RAJTVONAL\\season2\\database\\SM\\BOP_output.txt'):
...
def calculate_bop(position=1, point=0, leader_points=0, current_race=1, top3_results=0):
    ...
    dominance = point / ( MAX_POINTS_PER_RACE * races_done)         # 0-0.9
    gap_to_first       = point / leader_points   if leader_points > 0    else 0     # 0-1
    DOM = 0.5 * dominance + 0.5 * gap_to_first

    # --- SZEZON ELŐREHALADÁS
    SZE = current_race / TOTAL_RACES    # 0-1

    # --- POZÍCIÓ
    POZ = 1 #1 - ( ( position - 1 ) / ( PARTICIPANTS - 1 ) )

    # --- PÁLYAHOSSZ
    TRA = 1 # TRACKS[current_track][LENGTH] / AVG_TRACK_LENGTH      # 0.7 - 1.2

    # --- KARAKTERISZTIKA
    TYP = 1 #track_type_map[TRACKS[current_track][TYPE]]   

    # --- PONTKÜLÖNBSÉG
    delta = leader_points - point
    h = 30
    GAP = math.exp(-delta / h)   if delta > 0    else 1

    # --- CATCH-UP
    C_UP = 1 - math.exp(-3 * SZE)       # 0- 0.95

    # --- KONZISZTENCIA
    KON = 1 #1 + ( 0.1 * top3_results / current_race )

    # --- ÜZEMANYAGHASZNÁLAT
    FUL = 1 #TRACKS[current_track][FUEL]

    # --- ELŐZHETŐSÉG
    OVT = 1 # 1 + (0.5 - TRACKS[current_track][OT])
...
if __name__ == '__main__':
    ...
    save_bops(bops, 'C:\\Users\\Dani\\Desktop\\RAJTVONAL\\SimMasters\\database\\SM\\BOP_output.csv')
